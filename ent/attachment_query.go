...
func (_q *AttachmentQuery) QueryMedia() *MediaQuery {
...
func (_q *AttachmentQuery) QueryAttachmentJobs() *AttachmentJobQuery {
...
func (_q *AttachmentQuery) WithMedia(opts ...func(*MediaQuery)) *AttachmentQuery {
	query := (&MediaClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) WithAttachmentJobs(opts ...func(*AttachmentJobQuery)) *AttachmentQuery {
	query := (&AttachmentJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := _q.withMedia; query != nil {
		if err := _q.loadMedia(ctx, query, nodes,
			func(n *Attachment) { n.Edges.Media = []*Media{} },
			func(n *Attachment, e *Media) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAttachmentJobs; query != nil {
		if err := _q.loadAttachmentJobs(ctx, query, nodes,
			func(n *Attachment) { n.Edges.AttachmentJobs = []*AttachmentJob{} },
			func(n *Attachment, e *AttachmentJob) { n.Edges.AttachmentJobs = append(n.Edges.AttachmentJobs, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedMedia {
		if err := _q.loadMedia(ctx, query, nodes,
			func(n *Attachment) { n.appendNamedMedia(name) },
			func(n *Attachment, e *Media) { n.appendNamedMedia(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedAttachmentJobs {
		if err := _q.loadAttachmentJobs(ctx, query, nodes,
			func(n *Attachment) { n.appendNamedAttachmentJobs(name) },
			func(n *Attachment, e *AttachmentJob) { n.appendNamedAttachmentJobs(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AttachmentQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Media)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(media.FieldAttachmentID)
	}
	query.Where(predicate.Media(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attachment.MediaColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.AttachmentID
		if fk == nil {
			return fmt.Errorf(`foreign-key "attachment_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *AttachmentQuery) loadAttachmentJobs(ctx context.Context, query *AttachmentJobQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *AttachmentJob)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Attachment)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attachmentjob.FieldAttachmentID)
	}
	query.Where(predicate.AttachmentJob(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attachment.AttachmentJobsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *AttachmentQuery) WithNamedMedia(name string, opts ...func(*MediaQuery)) *AttachmentQuery {
	query := (&MediaClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedMedia == nil {
		_q.withNamedMedia = make(map[string]*MediaQuery)
	}
	_q.withNamedMedia[name] = query
	return _q
}

// WithNamedAttachmentJobs tells the query-builder to eager-load the nodes that are connected to the "attachment_jobs"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *AttachmentQuery) WithNamedAttachmentJobs(name string, opts ...func(*AttachmentJobQuery)) *AttachmentQuery {
	query := (&AttachmentJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
