...
func (_q *MediaQuery) QueryAttachment() *AttachmentQuery {
...
func (_q *MediaQuery) QueryAttachmentJobs() *AttachmentJobQuery {
...
func (_q *MediaQuery) WithAttachment(opts ...func(*AttachmentQuery)) *MediaQuery {
	query := (&AttachmentClient{config: _q.config}).Query()
...
func (_q *MediaQuery) WithAttachmentJobs(opts ...func(*AttachmentJobQuery)) *MediaQuery {
	query := (&AttachmentJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := _q.withAttachment; query != nil {
		if err := _q.loadAttachment(ctx, query, nodes, nil,
			func(n *Media, e *Attachment) { n.Edges.Attachment = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAttachmentJobs; query != nil {
		if err := _q.loadAttachmentJobs(ctx, query, nodes,
			func(n *Media) { n.Edges.AttachmentJobs = []*AttachmentJob{} },
			func(n *Media, e *AttachmentJob) { n.Edges.AttachmentJobs = append(n.Edges.AttachmentJobs, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedAttachmentJobs {
		if err := _q.loadAttachmentJobs(ctx, query, nodes,
			func(n *Media) { n.appendNamedAttachmentJobs(name) },
			func(n *Media, e *AttachmentJob) { n.appendNamedAttachmentJobs(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *MediaQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*Media, init func(*Media), assign func(*Media, *Attachment)) error {
...
func (_q *MediaQuery) loadAttachmentJobs(ctx context.Context, query *AttachmentJobQuery, nodes []*Media, init func(*Media), assign func(*Media, *AttachmentJob)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attachmentjob.FieldMediaKey)
	}
	query.Where(predicate.AttachmentJob(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(media.AttachmentJobsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "media_key" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *MediaQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withAttachment != nil {
			_spec.Node.AddColumnOnce(media.FieldAttachmentID)
		}
	}
...
func (_q *MediaQuery) WithNamedAttachmentJobs(name string, opts ...func(*AttachmentJobQuery)) *MediaQuery {
	query := (&AttachmentJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
